...
from CaptureConfig import LIVE_IMAGE_PATH, MODEL_IMAGE_SIZE
...
class Dinosaur(NeuralNetwork):
    ...
    def __init__(self, id, mask_color = None, autoplay = False):
        ...
        model_file = [MODEL_PATH] if os.path.exists(MODEL_PATH) else glob.glob('*.h5')
        if model_file:
    ...
    def predict(self):
        ...
        img = load_img(LIVE_IMAGE_PATH, color_mode='grayscale', target_size=MODEL_IMAGE_SIZE)
